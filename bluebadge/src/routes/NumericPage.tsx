...
export default function NumericPage() {
  const items = useMemo(() => shuffle(getCognitive('numeric')), [])
  ...
  const submit = () => {
    ...
    recordAttempt('numeriek', correct)
    setLocked(true)
  ...
  if (!current) {
    return <div className="card"><h2>Numeriek redeneervermogen</h2><p>Geen items gevonden. Importeer 50+ items via Admin Import (Cognitief).</p></div>
  }

  return (
    <div className="grid">
      <div className="card">
        <h2 style={{ marginTop: 0 }}>Numeriek redeneervermogen</h2>
        <p>{current.prompt}</p>
